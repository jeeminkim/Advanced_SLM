...
public class Join extends URLConnect {
    ...
    public void codesend (View v){
        ...
        if(emailcode.length() > 6) {
            AddData1(emailcode);
            AlertDialog.Builder ad=new AlertDialog.Builder(Join.this);
            ad.setTitle("이메일 인증");
            ad.setMessage("어플을 나가지 않아도 인증메일을 확인할 수 있습니다.");
            ad.setPositiveButton("네이버", new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {
                    Intent intent = new Intent(getApplicationContext(), auth.class);
                    startActivity(intent);
                }
            });
            ad.setNeutralButton("구글", new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {
                    Intent intent = new Intent(getApplicationContext(), authgoogle.class);
                    startActivity(intent);
                }
            });
            ad.setNegativeButton("다음", new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {
                    Intent intent = new Intent(getApplicationContext(), authdaum.class);
                    startActivity(intent);
                }
            });
            ad.create();
            ...
            Toast.makeText(getApplicationContext(),"이메일을 정확하게 주세요",Toast.LENGTH_SHORT).show();
        }
    }
    @Override
