...
public class Cart extends Parking {
    ...
    private class ListAdapter extends ArrayAdapter<MyData> {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            holder.check.setOnClickListener(new Button.OnClickListener() {
                public void onClick(View v) {
                    CheckBox checkBox;
                    checkBox = (CheckBox) findViewById(R.id.checkBox2);
                    if (checkBox.isChecked()) {
                        sql.execSQL("update " + CARTNAME + " set itemcheck = '1' where itemname = '" + tems.name + "'");
                        Toast.makeText(getApplicationContext(), "선택됨", Toast.LENGTH_SHORT).show();
                    } else if (!checkBox.isChecked()) {
                        sql.execSQL("update " + CARTNAME + " set itemcheck = '0' where itemname = '" + tems.name + "'");
